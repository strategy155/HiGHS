...
# MKL detection priority:
# 1. MKLROOT already set (Intel setvars.sh)
# 2. CSCS hipo-mkl environment (from setup_cscs.sh)
# 3. Spack (if available)
...
if [[ -n "${MKLROOT:-}" ]]; then
  ...
  # shellcheck source=/dev/null  # Auto-generated by setup_cscs.sh
  ...
  echo "ERROR: MKL not found." >&2
  echo "On CSCS: run ./scripts/setup_cscs.sh first" >&2
  echo "Otherwise: source Intel setvars.sh or install via spack" >&2
  exit 1
fi
